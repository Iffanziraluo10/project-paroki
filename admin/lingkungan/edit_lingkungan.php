...
<head>
    <meta charset="utf-8">
    <title>Data Lingkungan | Admin</title>
...
<body>
    <div class="container-xxl position-relative bg-white d-flex p-0">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            ...
                            <a href=".././lingkungan.php" class="dropdown-item active"><i class="fa fa-home"></i> Data Lingkungan</a>
                        </div>
                    </div>
        ...
        <div class="content">
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-light rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">Edit Data Lingkungan</h6>
                        <a href="../lingkungan.php" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Kembali</a>
                    </div>
                    <?php
                    $id = $_GET['id'];
                    $data = mysqli_query($koneksi,"SELECT * from lingkungan where id_lingkungan='$id'");
					while($d = mysqli_fetch_array($data)){
                    ?>
                    <form action="" method="POST" name="Form" onsubmit="return validateForm()">
                        <div class="mb-3">
                            <input class="form-control" type="hidden" name="id" value="<?php echo $d['id_lingkungan']; ?>">
                            <label for="" class="form-label">Nama Lingkungan</label>
                            <input type="text" class="form-control" name="namalin" value="<?php echo $d['nama_lingkungan'] ?>">
                        </div>
                        <div class="mb-3">
                            <label for="select-option" class="form-label">Nama Stasi / Wilayah</label>
                            <select class="form-select mb-3" id="select-option" name="nama_stasi" onchange="getSelectedOption()">
                                <option value="">--Silahkan Pilih--</option>
                                <?php
                                $query = "SELECT * FROM lingkungan inner join stasi where id_lingkungan=$id";
                                $result = mysqli_query($koneksi, $query);
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ?>
                                    <option value="<?php echo $row['id_stasi']; ?>"><?php echo $row['nama_stasi']; ?></option>
                                    <?php
                                }
                                ?>
                            </select>
                            <input type="hidden" name="stasi" class="form-control" id="stasi1" readonly>
                        </div>
            ...
            <?php
            if(isset($_POST['edit'])){
                $id_lingkungan = $_POST['id'];
                $namalin = $_POST['namalin'];
                $stasi = $_POST['stasi'];
                
                $query_edit = mysqli_query($koneksi, "UPDATE lingkungan SET nama_lingkungan='$namalin', id_stasi='$stasi' where id_lingkungan='$id_lingkungan'");
    ...
    <script>
        function validateForm() {
            var a = document.forms["Form"]["namalin"].value;
            var b = document.forms["Form"]["nama_stasi"].value;
